...
describe('FormatString: ', () => {
  ...
  it('should return dict response', () => {
    ...
    f.generate()
    expect(f.response).toMatchObject({ name: 'string', age: 'number' })
  })
  it('should return list with two dict', () => {
    ...
    expect(f.response).toMatchObject(expected)
  })
})
describe('ResponseDict: ', () => {
  ...
  it('should generate json with single value', () => {
    ...
    f.generate()
    expect(f.response).toMatchObject({ name: 'string' })
  })
...
describe('ResponseList: ', () => {
  ...
  it('should generate array with 2 responses', () => {
    ...
    expect(res.response).toMatchObject(expected)
  })
...
describe('ValueGenerator: ', () => {
  ...
  it('should return staic value', () => {
    ...
    expect(v.generate('string')).toBe('string')
    expect(v.generate('number')).toBe('number')
  })
  it('should return json as response', () => {
    const v = new ValueGenerator()
    const storage = DataStorage.getInstance()
    storage.add('abc', { name: 'jhon', age: '18' })
    expect(v.generate(VAR_CHILD_IDENTIFIER + 'abc')).toMatchObject({
      name: 'jhon',
      age: '18',
    })
  })
