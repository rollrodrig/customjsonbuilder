...
describe('Block: ', () => {
  it('should replace sub patterns', () => {
    ...
    expect(newPattern).toBe('{name:string,birth:abc123}')
  })
  it('should replace two sub patterns and keep the backup pattern', () => {
    const pattern = '{name:string,birth:{city:string},email:{bussines:string}}'
    const block = new Block(pattern)
    let newPattern = block.replaceSubPatterns('{city:string}', 'b1')
    expect(newPattern).toBe('{name:string,birth:b1,email:{bussines:string}}')
    expect(block.lockedPattern).toBe(
      '{name:string,birth:{city:string},email:{bussines:string}}',
    )
    newPattern = block.replaceSubPatterns('{bussines:string}', 'c1')
    expect(newPattern).toBe('{name:string,birth:b1,email:c1}')
    expect(block.lockedPattern).toBe(
      '{name:string,birth:{city:string},email:{bussines:string}}',
    )
  })
  it('blocks should work together', () => {
    ...
    expect(blockB.pattern).toBe('{day:string,year:c,month:{name:string}}')
    blockB.replaceSubPatterns(blockD.lockedPattern, 'd')
    expect(blockB.pattern).toBe('{day:string,year:c,month:d}')
    blockA.replaceSubPatterns(blockB.lockedPattern, 'b')
    expect(blockA.pattern).toBe('{name:string,birth:b}')
  })
  it('should generate json response', () => {
    ...
    const block = new Block(pattern)
    expect(block.generate()).toMatchObject({ name: 'string', city: 'string' })
    stub1.restore()
  })
